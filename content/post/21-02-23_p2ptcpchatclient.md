...
As part of their coursework, each student was required to create a TCP P2P (peer-to-peer) chat client using C++ OOP style. This involved creating a chat client that could connect with one other peer, with the IP address provided as a command line argument. The first peer, however, did not require an argument. Once connected, the chat client needed to be able to receive a list of other peers from the first peer, listen and accept incoming peers, and send the list of peers to any new peers that connected to the network. Finally, the chat client needed to be capable of sending and receiving messages to and from all other peers in the network. Overall, this project required students to demonstrate their understanding of fundamental concepts in network programming, as well as their ability to apply these concepts in a practical project.
...
However, I do wonder how the rest of the class fared on this project. The only time I got to see what anyone else did was when I asked them in class. No other public repositories like mine were created, and I don't blame anyone, since doing so could have resulted in their code getting "borrowed". I didn't really mind since creating this project wasn't even required. If one person, in this case, me, created the project, the other students were free to use it. This doesn't bother me since the next assignment is individual anyway (this one is on 10%, the next one on 40%), so this was just an introduction to the other project for me.
